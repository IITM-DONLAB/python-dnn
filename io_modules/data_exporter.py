...
class  T1DataExporter(DataExporter):
	...
	def dump_data(self,out_fn,out_featdim):
		...
		shutil.copy(filepath,self.export_path);	#copies the file directly
...
class  T2DataExporter(DataExporter):
	...
	def dump_data(self,out_fn,out_featdim):
		...
		for idx in xrange(num_classes):
			level1_filepath  = self.data_spec['base_path'] + os.sep + self.filehandle.readline().strip()	#filename of individual classes
			shutil.copy(level1_filepath,self.export_path);	#copies the index file directly from the source directly
